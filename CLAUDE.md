...
**`runEffect(name, opts)`** dispatches STRUCTURE effects: `'ascii'` → `applyASCII`, `'erode'` → `applyGLFilter('erode', ...)`. STRUCTURE shaders share an `applyStructureOutput(structure, src, mode)` helper (copy-pasted into each FRAG) whose numeric contract is four modes — `mono` (0, grayscale on black), `source` (1, mask the source RGB), `ink` (2, black/cream poster via `uInkLow`/`uInkHigh`), `invert` (3, negative of mono). **ColorIso is not a real 5th shader mode** — it's a JS-side pre-pass: when selected, the render path runs the `colorisolation` shader over the input first, then forces the actual `applyStructureMode` call to numeric `source` (1) for final compositing; the `colorisolation` key in each `STRUCTURE_OUTPUT_MODE_VALUE` map is a routing sentinel, not a value the shader ever sees. The two pipelines expose different option sets from that same 4-mode shader contract: main synth's `structure-output-group` is Source/Mono/Invert/ColorIso (`STRUCTURE_OUTPUT_MODE_VALUE = { mono:0, source:1, colorisolation:2, invert:3 }` in `main.js`, top-level) — Ink was dropped from the main picker. The blob `blob-structure-output-group` keeps all five conceptually (Mono/Source/Ink/Invert/ColorIso, `{ mono:0, source:1, ink:2, invert:3, colorisolation:4 }` inside `resolveBlobPipeline`), with `colorisolation` routed the same pre-pass way. The string→number maps live in `main.js`; a new *shader* mode must be added to every copy of the helper, but a new *pre-pass routing* mode (like ColorIso) only needs a sentinel value + branch in the render path. **Critical**: some shaders exist in TWO variants — an FX RACK version (no `applyStructureOutput`) and a blob-structure version (with it). `edgedet` is the current case: `FRAG_EDGEDET` (FX rack) has the colored hue-glow output; `FRAG_EDGEDET_STRUCT` (exported from `glFilters.js`) is the structure-mode variant used by the blob pipeline via `BLOB_STRUCT_FRAG_OVERRIDES` in `glBlobPipeline.js`. When adding a new blob STRUCTURE effect that shares a name with an FX RACK shader, add it to that override map rather than modifying the FX shader.
...
Two OPTIONAL uniforms are auto-wired by the dispatchers (`applyGLFilter` AND `applyFxEffect`, plus their glBlobPipeline.js equivalents) — declare them and they work; omit them and they cost nothing (cached location is null, upload skipped):
- `uniform float uTime` — seconds, for animated effects (GRADE's Hue Rate/Range cycle, sequin, octopus, hologram, dreamstatic, crtrolling, wobbletape). Sourced from `getShaderTimeSeconds()` in `glUtil.js`, NOT `performance.now()` directly: live playback reads real wall-clock time, but offline export overrides it to each exported frame's own virtual timestamp via `setShaderTimeOverride(t)` (set per-frame in `_exportWebCodecs`/`_exportPNGZip`, cleared in `startOfflineExport`'s `finally`). Export renders frames as fast as seek/draw/encode allow, not in real time, so a uTime-driven effect paced by `performance.now()` would animate at "how fast the export happens to run" instead of the video's own timeline — visibly sped-up/uneven at the start (cold shader compiles, first seeks) then settling once the render loop's pace stabilizes. All four uTime/time upload sites (`glFilters.js`, `glFx.js`, `glBlobPipeline.js` ×2 for its stateless + feedback dispatchers) go through this one shared clock, so the fix is automatic in both the main pipeline and the blob/TRACK pipeline.
...
- `uniform float uParam4` — an optional 5th scalar for effects that genuinely need more than the 4 `uParams` slots (escape hatch from the 4-knob house pattern). BOTH dispatchers (`applyGLFilter` and `applyFxEffect`) upload `params[4]` when the uniform exists and a 5th value is passed; pass a 5-element `order` array (runEffect/runFxEffect map the whole order and only pad to 4). Used by `lumadrag` (wobble, feedback FX in glFx) and `moddiff` (drift). Prefer keeping to 4 knobs; reach for this only when a 5th control is clearly warranted. (Shader SOURCES are separate — they use a `uParams[8]` array, see below.)
...
| `dither` | `glFilters.js` | x=scale(1–8px cell), y=levels(2–8), z=gamma, w=bias | STRUCTURE. Bayer 4×4 ordered dithering — quantizes luma to N gray levels via halftone matrix. Defaults: scale=0.5, levels=0.25. 1-bit B&W in Ink mode; classic retro-game look. |
| `colorisolation` | `glFilters.js` | x=hue(0–1), y=overlap(width), z=steepness, w=mode(0=isolate/1=reject) | STRUCTURE. Gaussian hue-window mask — outputs bright where pixels match the target hue (Isolate) or the inverse (Reject). Weighted by saturation so grays fall through. |
| `kuwahara` (struct) | `glFilters.js` (`FRAG_KUWAHARA_STRUCT`) | x=radius(1–5px), y=sharpness | STRUCTURE. 7×7 quadrant Kuwahara filter output as luma — painterly stroke regions as a structure mask. No sat boost/blend. Dispatched via `kuwahara_struct` key. |
| `veins` | `glFilters.js` | x=Strength, y=Scale, z=Detail, w=Bend | STRUCTURE. Adapted from Rune Skovbo Johansen's Phacelle Noise (2D simplification): seeds directional cell noise from the source's own luma gradient, then up to 3 octaves each bending direction off the previous octave's output — organic branching vein/root/crack patterns rather than uniform grain. |
| `sketch` | `glFilters.js` | x=Ink, y=Stroke, z=Wobble, w=Speed, `uParam4`=Bg | STRUCTURE (also a blob STRUCTURE effect). sofiene71 "notebook drawings" ray-march shader (CC BY-NC-SA 3.0, shadertoy.com/view/ldlcWs), single-sided march at 3 angles × 9 samples. Bg=0 → clean grayscale (Mono mode); Bg=1 → warm sepia ink pass. Uses `uTime` for Wobble/Speed animation. |
| `subsurface` | `glFilters.js` | x=Scatter, y=Material(0=skin/0.5=wax/1=marble), z=Translucency, w=Back Light | COLOR/UNIQUE (Material category). Ported from a TouchDesigner GLSL TOP — vertical light-scatter sample bleeds downward from bright areas, tinted per Material palette (skin=warm red, wax=amber, marble/jade=green); Back Light inverts so dark areas glow (light from behind). |
| `edgedet` | `glFilters.js` | x=thresh, y=glow(0=soft/1=sharp), z=hue, w=blend(0=on-src/1=on-black) | FX RACK: Sobel edges as colored hue glow overlaid on source. NO `applyStructureOutput` — used only from the FX rack. |
| `edgedet` (struct) | `glFilters.js` (exported as `FRAG_EDGEDET_STRUCT`) | x=thresh, y=glow — z/w unused in struct mode | Blob STRUCTURE variant: same Sobel kernel, routes through `applyStructureOutput` so mono/source/ink/invert all work. Compiled by the blob pipeline via `BLOB_STRUCT_FRAG_OVERRIDES`. |
...
| `src/glUtil.js` | `uploadVideoTexture` — allocate-once + `texSubImage2D` fast-path (saves ~8 MB GPU alloc/free per frame at 1080p); also handles UNPACK_FLIP_Y_WEBGL. Also owns the shared shader clock (`getShaderTimeSeconds`/`setShaderTimeOverride`) that all `uTime` uploads read from — see the uTime uniform note above |
...
- **UNIQUE** — effects that BUILD something (neighbor sampling, added
  ...
  aurorastorm, subtleaurora, deepfield, dreamstatic, quantumstate), Light
  (neontube, prismatic, heatbleed, sequin, hologram, tokaplasma,
  irishell), Dimension (depthstack, abyss), Material (subsurface — see
  Shaders by effect below), Deep Sea (octopus), Print (risograph,
  newsprint, hatch, okband), Motion (predator — uses u_prev), Painterly
  (colorfulposter, cospalette). Still stateless single-frame passes —
...
See `lumisynthprd.md` for implementation status vs the original PRD. P2 (STRUCTURE → COLOR FBO chain) shipped. Track FX rack (echo/radar/heatmap) is implemented in TRACK mode. Curved hub lines are implemented as a general TRACK Lines style (`hubcurve`). Cloudflare Pages Functions + D1 auth/presets/export-gating scaffolding exists, with localhost-only internal login for testing before real D1 setup. P3 is WELL UNDERWAY: the FX RACK is a real GL rack (3 slots, drag-reorder, per-slot params, timeline-look + preset + persistence integration) with seven true feedback effects (`rgbdelay`, `flowfield`, `drag`, `lumadrag`, `tunnel`, `burnin`, `wobbletape`) plus the stateless signal/texture set (bloom, godrays, decayflow, feedbackwarp, crt, crtrolling, scanlines, degrade, noise) — see `src/glFx.js`. v8 replaced the COLOR rack with the single tabbed COLOR stage (MAPS / UNIQUE / CUSTOM + GRADE). June 2026 additions: the dreamcore COLOR pack (octopus, hologram, surveil, newsprint, sketch, polaroid, blacklight, dreamstatic, predator + earlier blackbody/hubble/abyss/sequin/risograph), the motion infrastructure (frame-history ring + `u_prev`), the `uTime` auto-upload convention in both dispatchers, the single-bar timeline, the TE-cream chrome redesign, and the generative SHADER LIBRARY source kind (`src/shaderSource.js` — `diveclouds`, `phantomstar`, `starnest`, `hyperkart`; raymarched/Shadertoy-style generators with registry-driven per-shader knobs: Speed glides via an accumulated phase clock, others upload via a `uParams[8]` float array, up to 8 controls each). **Blob LumiSynth is fully implemented** (`src/glBlobPipeline.js`) — per-blob independent STRUCTURE→COLOR→GRADE→FX chain composited source-over on the display canvas, rectangular blob bboxes from gap-tolerant directional scan, `category` field propagated through Kalman tracker, unified `trackLabels: 'off'|'confidence'|'position'` replacing separate XY-labels and MediaPipe-labels sections, MediaPipe hardcoded to GPU delegate, lag cursor permanently disabled. **Blob FX rack now supports all 16 effects** including feedback — glBlobPipeline.js imports `FX_FRAGS` from glFx.js and runs its own ping-pong FBO pairs in its private `_gl` context; `resetBlobFeedback` wired into all slot mutations and `resetAllState`. **ASCII edge detection** added: Sobel filter at cell level quantizes gradient orientation to 4 bins → `_`/`/`/`|`/`\` glyphs override density glyphs when `uEdgeThreshold > 0`; controlled by the `Edges` slider (`asciiEdgeThreshold`). **RGBDelay** FX added — per-channel feedback persistence (R short, B long) + spatial drift that orbits channel UV samples, creating chromatic ghost trails on motion. **DoG** (Difference of Gaussians) and **Dither** (Bayer 4×4 ordered) added to the STRUCTURE stage — DoG isolates contours via two-sigma subtraction (anime line-art in Invert, pencil-sketch in Ink); Dither quantizes luma to 2–8 gray levels via a halftone matrix (1-bit print look in Ink). Both available in the blob STRUCTURE picker too. **AcerolaFX-inspired effects** (June 2026): 8 new FX RACK stateless effects (`vignette` radial darkening, `tonemap` Reinhard/ACES/Hable HDR, `chromab` R/G/B channel split, `sharpen` unsharp mask, `edgedet` Sobel edge overlay with hue-colored glow, `bokeh` 12-sample ring blur with bright-highlight weighting, `filmgrain` animated Gaussian grain with shadow bias + halation, `autoexp` feedback auto-exposure via corner-pixel state storage), plus 4 new COLOR effects: `palswap` and `csadjust` in MAPS (OKLCH-based luma→hue remap and direct L/C/H/warmth knobs), `halftone` (CMYK 4-angle dot screens) and `colorfulposter` (logistic luma posterization + CMYK hard-light color layer) in UNIQUE/Print and UNIQUE/Painterly respectively (`halftone` was later removed and replaced by `hatch` — see below). Remaining P3: upgrading decayflow/feedbackwarp to real feedback, and Inv/Thermal moving from PER-BLOB into the FX RACK. `PRD_DECISIONS.md` logs what is deliberately out of scope. **Recent fixes and additions**: Rate auto-randomize for okdrift now calls `schedulePersist()` instead of the no-op `activateColor()`, fixing the Rate knob not updating the GL output. `resolveTimelineLook` returns live `state` for video without segments (was returning a stale blank `makeRawTimelineLook()`). Export recording upgraded to H.264 High Profile (avc1.640034/640032) with 25/80/200 Mbps bitrates for 720p/1080p/4K. Okdrift harmony modes expanded to 10 (added Split-Comp, Spectral, Duotone, Pentadic — modes 6–9); uParam4 packing `N*10 + relType` with clamp updated to 9 in both JS and GLSL. **IGN** (Interleaved Gradient Noise) added to FX RACK — Jorge Jimenez's low-discrepancy grain formula with golden-ratio temporal animation, 4 knobs (Amount/Scale/Posterize/Chromatic). **Blob edgedet output modes fix**: `edgedet` as a blob STRUCTURE effect was ignoring `uOutputMode` because `FRAG_EDGEDET` (FX rack shader) has no `applyStructureOutput`. Added `FRAG_EDGEDET_STRUCT` (exported from `glFilters.js`) — same Sobel kernel routed through `applyStructureOutput` — and `BLOB_STRUCT_FRAG_OVERRIDES` in `glBlobPipeline.js` to use it; FX rack edgedet (colored hue glow) unchanged. **STRUCTURE overhaul** (June 2026): `freqmod` removed; replaced with three new STRUCTURE effects — `colorisolation` (Gaussian hue-window mask, Isolate/Reject modes), `cartoon` (diagonal 4-sample Sobel edge strength), and `kuwahara` (7×7 quadrant Kuwahara luma structure, dispatched via `kuwahara_struct` key to avoid collision with the old COLOR kuwahara shader). **COLOR overhaul**: `kuwahara` (painterly quadrant filter) removed from COLOR/Painterly and replaced by `colorfulposter` (logistic luma posterization + CMYK hard-light color layer, 4 knobs: Levels/Slope/Cont/Tint). **New FX RACK effect**: `fakehdr` — dual 8-sample bloom at two radii, difference drives a local-contrast expansion (CeeJay.dk FakeHDR port), 4 knobs (Power/Near/Far/Mix). (Both `colorisolation` and `cartoon` were later removed as standalone STRUCTURE picker effects — `cartoon` was deleted outright, and `colorisolation` was converted into the ColorIso output-mode pre-pass described above. Current `STRUCTURE_SECTIONS` is `['ascii', 'erode', 'pixelsort', 'melt', 'motionedge', 'edgedet', 'dither', 'veins', 'kuwahara', 'moddiff', 'sketch']`.)
**Late June 2026 additions** (previously undocumented here): `veins` — a new STRUCTURE effect adapted from Rune Skovbo Johansen's Phacelle Noise terrain-erosion technique (2D simplification): directional cell noise seeded from the source's own luma gradient, up to 3 bending octaves, producing organic branching vein/root/crack patterns (4 knobs: Strength/Scale/Detail/Bend). `sketch` — a new STRUCTURE effect (sofiene71 "notebook drawings" ray-march shader) available both as a main STRUCTURE pick and a blob STRUCTURE pick, with Bg=0 clean grayscale / Bg=1 warm sepia ink. `hatch` replaced `halftone` in COLOR UNIQUE/Print — hlorenzi sine-crosshatch driven by video luma (Density/Jitter/Paper/Blend knobs). ColorIso is now also a **blob** STRUCTURE **output mode** (`blobStructureOutputMode: 'colorisolation'`, numeric 4 in `resolveBlobPipeline`'s `STRUCTURE_OUTPUT_MODE_VALUE`), mirroring the main synth's pre-pass approach — it runs the hue-window filter on the raw source before the blob's structure shader, then blends with `mode=1` (source), not `mode=0` (mono/greyscale-only), so isolated-hue color survives instead of being stripped. `setTrackFxSlotType`/`setBlobFxSlotType` now auto-expand the picked slot's knob panel on selection (previously only handled collapsing, never expanding, unlike the synth FX rack's `setFxSlotType`).
**This session**: added `subsurface` — a Subsurface Scatter COLOR effect (new Material category in `COLOR_UNIQUE_SECTIONS`) ported from a TouchDesigner GLSL TOP; see Shaders by effect above. Also fixed a real-time-vs-export desync bug: OKDrift's Rate-driven hue auto-randomize and GRADE's Hue Rate/Range cycle (plus every other `uTime`-driven animated effect) were paced by `performance.now()`, which runs at a different rate than offline export's frame-stepped seek/draw/encode loop — exported animation looked sped-up at the start of a render, then "caught up" to the intended pace once the loop's per-frame rate stabilized. OKDrift's JS-side random-hue state is now advanced by `_advanceOkdriftForExport` using each exported frame's own virtual timestamp instead of wall-clock time (wired into `_renderOneFrame` for both the main COLOR and blob COLOR pipelines, gated by a new `_isExporting` flag that silences the live panel's real-time randomizer during export). Every GPU `uTime` upload (`glFilters.js`, `glFx.js`, `glBlobPipeline.js` ×2) now reads a shared `getShaderTimeSeconds()` in `glUtil.js`, which the export loop pins to the frame's virtual time via `setShaderTimeOverride(t)` — this fixes GRADE's hue cycler and every other animated shader consistently in both the main synth pipeline and the blob/TRACK pipeline, with one shared mechanism instead of a per-effect patch. **Performance**: `detectBlobs` Float32Array buffers are now cached (no per-frame GC allocation). **Blob synth fix**: blob composite now uses `globalAlpha` scaled to blob presence so low-confidence detections fade naturally. **Blob proc tab fix**: `blobColorProcGrid` DOM element removed from index.html; `main.js` sets it to `null` — proc tab auto-selects okdrift directly, no swatch grid needed. Blob-side okdrift panel (`buildBlobOkdriftPanel`) mirrors the main okdrift panel with its own `_blobOkdriftAnimId`/`_blobOkdriftLastRand` state. **Track STRUCTURE sync**: the track-mode blob STRUCTURE picker now stays in sync with the synth STRUCTURE picker on tab changes. **Dither defaults updated**: scale=0.5 (was 0.4), levels=0.25 (was 0.3). **moddiff Freq knob range extended** to max:3 (was max:1).
