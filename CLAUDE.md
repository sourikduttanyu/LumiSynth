...
- `uniform float uTime` — seconds (`performance.now()/1000`), for animated effects (sequin, octopus, hologram, dreamstatic, crtrolling, wobbletape)
...
| `cartoon` | `glFilters.js` | x=power(edge intensity), y=slope(edge threshold) | STRUCTURE. Diagonal 4-sample Sobel edge detection — outputs edge strength as raw structure float. Invert mode = dark outlines on white. |
