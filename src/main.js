...
function runEffect(name, opts) {
  ...
  switch (name) {
    ...
    case 'orbdiff':
      return applyGLFilter('orbdiff', canvas.width, canvas.height, [look.orbdiffScale, look.orbdiffSharp, look.orbdiffDiffuse, look.orbdiffThresh], { ...opts, outputMode, ...inkColors });
    case 'oxide':
    case 'synth':
    case 'biolum':
    case 'thermo':
...
function resolveBlobPipeline(look) {
  ...
  if (structureName) {
    ...
    switch (structureName) {
      ...
      case 'orbdiff':   structureParams = [p.scale ?? 0.25, p.sharp ?? 0.4, p.diffuse ?? 0.35, p.thresh ?? 0.08]; break;
